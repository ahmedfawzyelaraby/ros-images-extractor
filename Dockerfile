...
RUN sh -c 'echo "deb http://packages.ros.org/ros/ubuntu $(lsb_release -sc) main" > /etc/apt/sources.list.d/ros-latest.list' && \
    ...
    apt -qq update && apt -y -qq install ros-melodic-desktop-full && rosdep init && rosdep update && \
    apt -y -qq install python-rosinstall python-rosinstall-generator python-wstool build-essential && apt clean